...
#                 self.counter[word] += 1
    ...
    def index_utter_words(self, min_freq=1):
...
def read_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    ...
    with open(file_name) as f:
        ...
        for dial_dict in dials:
            if (args["all_vocab"] or dataset in ["train", "augmented"]) and training:
        ...
        if training and dataset in ["train", "augmented"] and args["data_ratio"]!=100:
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    ontology = json.load(open(args['ont_path'], 'r'))
    ...
    if training:
        ...
        if os.path.exists(os.path.join(folder_name, lang_name)) and os.path.exists(os.path.join(folder_name, mem_lang_name)):
            ...
            print("[Info] Dumping lang files to %s..." % folder_name)
        ...
        emb_dump_path = args['emb_path'].format(len(lang.index2word))
        if not os.path.exists(emb_dump_path) and args["load_embedding"]:
            emb_dump_path = emb_dump_path.format(len(lang.index2word))
            print(emb_dump_path)
