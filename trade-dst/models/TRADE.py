...
class TRADE(nn.Module):
    ...
    def save_model(self, dec_type):
        directory = 'TRADE-'+args["addName"]+args['dataset']+str(self.task)+'/'+'HDD'+str(self.hidden_size)+'BSZ'+str(args['batch'])+'DR'+str(self.dropout)+str(dec_type)
        directory = os.path.join(args['output_path'], directory)
    ...
    def evaluate(self, dev, matric_best, slot_temp, early_stop=None):
        ...
        if args["genSample"]:
            output_path = os.path.join(args['output_path'], "all_prediction_{}.json".format(self.name))
            json.dump(all_prediction, open(output_path, 'w'), indent=4)
...
class EncoderRNN(nn.Module):
    def __init__(self, vocab_size, hidden_size, dropout, n_layers=1):
        ...
        if args["load_embedding"]:
            with open(args['emb_path'].format(vocab_size)) as f:
                E = json.load(f)
