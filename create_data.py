...
fin = open('neuralwoz/synthesize/mapping.pair','r')
replacements = []
...
def insertSpace(token, text):
    ...
    while True:
        ...
        if sidx == -1:
            break
        if sidx + 1 < len(text) and re.match(r"[0-9]", text[sidx - 1]) and \
...
def loadData(args):
    data_path = os.path.join(args.main_dir, "data.json")
    if not os.path.exists(args.main_dir):
        os.makedirs(args.main_dir)

    if not os.path.exists(data_path):
        print("Downloading and unzipping the MultiWOZ 2.1 dataset")
        url = "https://raw.githubusercontent.com/budzianowski/multiwoz/master/data/MultiWOZ_2.1.zip"
        response = requests.get(url)
        open(os.path.join(args.main_dir, "MultiWOZ_2.1.zip"), "wb").write(response.content)
        zip_file = zipfile.ZipFile(os.path.join(args.main_dir, "MultiWOZ_2.1.zip"))
        zip_file.extractall(args.main_dir)
        
        for file in os.listdir(os.path.join(args.main_dir, "MultiWOZ_2.1")):
            if file in ["data.json", "system_acts.json", "testListFile.txt", "valListFile.txt", "slot_descriptions.json"] or \
            file.endswith('_db.json'):
                shutil.move(os.path.join(args.main_dir, "MultiWOZ_2.1", file), os.path.join(args.main_dir, file))

        os.remove(os.path.join(args.main_dir, "MultiWOZ_2.1.zip"))
...
def createData(args):
    ...
    fin2 = open(os.path.join(args.main_dir, 'system_acts.json'), 'r')
...
def divideData(data, args):
    ...
    testListFile = []
    fin = open(os.path.join(args.main_dir, 'testListFile.txt'), 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open(os.path.join(args.main_dir, 'valListFile.txt'), 'r')
    for line in fin:
        valListFile.append(line[:-1])
    fin.close()

    trainListFile = open(os.path.join(args.main_dir, 'trainListFile.txt'), 'w')
    ...
    for dialogue_name in data:
        ...
        if dial:
            ...
            if dialogue_name in testListFile:
                collector_test[dialogue_name] = data[dialogue_name]
                test_dials.append(dialogue)
                count_test += 1
            elif dialogue_name in valListFile:
                collector_dev[dialogue_name] = data[dialogue_name]
                val_dials.append(dialogue)
                count_val += 1
            else:
                collector_train[dialogue_name] = data[dialogue_name]
                trainListFile.write(dialogue_name + '\n')
                train_dials.append(dialogue)
                count_train += 1
                # For leave-one-out setting
                train_domain_transitions[dialogue_name] = dialogue['domains']

    print("# of dialogues: Train {}, Val {}, Test {}".format(count_train, count_val, count_test))
    
    # save all dialogues for Collector
    with open(os.path.join(args.main_dir, 'collector_train_data.json'), 'w') as f:
        json.dump(collector_train, f, indent=4)
    
    with open(os.path.join(args.main_dir, 'collector_dev_data.json'), 'w') as f:
        json.dump(collector_dev, f, indent=4)
        
    with open(os.path.join(args.main_dir, 'collector_test_data.json'), 'w') as f:
        json.dump(collector_test, f, indent=4)
    
    # save all dialogues for Labeler
    with open(os.path.join(args.main_dir, 'labeler_dev_data.json'), 'w') as f:
        json.dump(val_dials, f, indent=4)

    with open(os.path.join(args.main_dir, 'labeler_test_data.json'), 'w') as f:
        json.dump(test_dials, f, indent=4)

    with open(os.path.join(args.main_dir, 'labeler_train_data.json'), 'w') as f:
        json.dump(train_dials, f, indent=4)
        
    with open(os.path.join(args.main_dir, 'train_domain_transitions.json'), 'w') as f:
        json.dump(train_domain_transitions, f, indent=4)
        
    shutil.copyfile('assets/ontology.json', os.path.join(args.main_dir, 'ontology.json'))
    shutil.copyfile('assets/fewshot_key.json', os.path.join(args.main_dir, 'fewshot_key'))
